...
@interface GJTabBar ()
/** selctButton */
@property (weak , nonatomic) GJTabBarButton *selButton;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if ([GJTabBarConfig shared].haveBorder) {
            self.border.fillColor = [GJTabBarConfig shared].bordergColor.CGColor;
        }
        self.backgroundColor = [GJTabBarConfig shared].backgroundColor;
        
        [[GJTabBarConfig shared]addObserver:self forKeyPath:@"textColor" options:NSKeyValueObservingOptionNew context:nil];
        [[GJTabBarConfig shared]addObserver:self forKeyPath:@"selectedTextColor" options:NSKeyValueObservingOptionNew context:nil];
    }
    return self;
}
...
- (void)setItems:(NSArray<UITabBarItem *> *)items{
    ...
    for (int i=0; i<items.count; i++)
    {
        ...
        {
            self.centerBtn = [GJCenterTabBarBtn buttonWithType:UIButtonTypeCustom];
        ...
        {
            btn = [GJTabBarButton buttonWithType:UIButtonTypeCustom];
            //Add Observer
            ...
            [self.btnArr addObject:(GJTabBarButton *)btn];
            [btn addTarget:self action:@selector(controlBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        }
...
- (CAShapeLayer *)border{
    if (!_border) {
        ...
        border.path = [UIBezierPath bezierPathWithRect:
                       CGRectMake(0,0,self.bounds.size.width,[GJTabBarConfig shared].borderHeight)].CGPath;
...
- (void)layoutSubviews{
    ...
    NSInteger mid = [GJTabBarConfig shared].centerBtnIndex;
    ...
    _border.path = [UIBezierPath bezierPathWithRect:CGRectMake(0,0,
                                                               ...
                                                               [GJTabBarConfig shared].borderHeight)].CGPath;
}
 ...
 */
- (void)controlBtnClick:(GJTabBarButton *)button{
...
- (void)setSelectButtoIndex:(NSUInteger)index{
    if (self.centerBtn && index == self.centerBtn.tag) {
        self.selButton = (GJTabBarButton *)self.centerBtn;
    }else{
        for (GJTabBarButton *loop in self.btnArr){
            if (loop.tag == index){
                self.selButton = loop;
                break;
            }
 ...
 */
- (void)setSelButton:(GJTabBarButton *)selButton{
 ...
 *  Center button click
 */
- (void)centerBtnClick:(GJCenterTabBarBtn *)button{
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"badgeValue"] || [keyPath isEqualToString:@"badgeColor"]){
        GJTabBarButton *btn = (__bridge GJTabBarButton *)(context);
        btn.item = (UITabBarItem*)object;
    }
    else if ([object isEqual:[GJTabBarConfig shared]]){
...
- (void)dealloc{
    ...
    [[GJTabBarConfig shared]removeObserver:self forKeyPath:@"textColor" context:nil];
    [[GJTabBarConfig shared]removeObserver:self forKeyPath:@"selectedTextColor" context:nil];
}
